...
(function() {
    milo.base.extend = function(destination, source) {
        if (destination == null) { destination = source } else {
    ...
    milo.base.extendLess = function(destination, source) {
        ...
        for (var i in destination) {
            if (isObject(source) && typeof source[i] != "undefined") { destination[i] = newopt[i] }
        }
        return destination
    };
    ...
    milo.base.extend(milo.base, {
        ...
        forEach: function(haystack, callback) {
            ...
            if (length !== undefined) {
                ...
                for (name in haystack) { callback.call(haystack[name], name, haystack[name]) }
            }
        },
        g: function(obj) {
...
(function() {
    var config = { loaderPath: "http://ossweb-img.qq.com/images/js/milo/", version: "20130701", expires: 3e4 };
...
(function(loader) {
    var __loading = null,
        ...
        modulemap = {};
    charset = "gb2312";
    loader.set = function(obj) { charset = obj.charset };
    ...
    loader.loadCSS = function(url, callback) {
        if (url.search(/^http:\/\//i) == -1) { url = milo.config.loaderPath + url.replace(/\./g, "/") + ".css" + "?" + milo.config.version }
    ...
    function moduleContainer(name, modules, callback) {
        ...
        return {
            ...
            completeLoad: function(maped) {
                var ret = [];
                for (var i = 0; i < this.modules.length; i++) { ret.push(this.res[this.modules[i]]) }
                if (!isFunction(this.callback) && !isObject(this.callback)) return false;
                if (this.name == "") { this.callback.apply(null, ret) } else { isObject(this.callback) ? modulemap[this.name] = this.callback : modulemap[this.name] = this.callback.apply(null, ret) }
            },
            checkMaped: function() {
                ...
                if (this.hasmaped < this.needown && this.expires > 0) {
                    ...
                    var mc = this;
                    setTimeout(function() { mc.checkMaped() }, 50)
                }
            }
        }
    }
    ...
    function start(mc) {
        var need = mc.need;
        for (var key in need) { load(need[key], key, mc) }
        checkloaded(mc)
    }
    ...
    function load(url, name, mc) {
        var isCSS = /\.css(\?|$)/i.test(url);
        loading[name] = true;
        isCSS ? loadCSS(url, function(ret) { mc.loadUrlCallback.call(mc, [ret, name]) }) : loadScript(url, function(ret) { mc.loadUrlCallback.apply(mc, [ret, name]) })
    }
    ...
    function checkloaded(mc) {
        ...
        if (mc.hasdown < mc.needown && mc.expires > 0) {
            ...
            setTimeout(function() { checkloaded(mc) }, 50)
        }
    }

    function getModulePath(filepath) {
        if (filepath.search(/^http:\/\//i) == -1) { filepath = milo.config.loaderPath + filepath.replace(/\./g, "/") + ".js" + "?" + milo.config.version }
        return filepath
    }
    ...
    function includerContainer(files, callback) {
        ...
        return {
            ...
            loadInluderCallback: function(ret) {
                var res = [];
                for (var i = 0; i < this.files.length; i++) { res.push(loaded[this.files[i]]) }
    ...
    function onload(node, callback) {
        ...
        if (document.addEventListener) { node.addEventListener("load", function() { callback.call(this, node) }, false) } else if (!isImpOnLoad) {
...
(function() {
    ...
    extend(dom, {
        browser: { version: (userAgent.match(/.+(?:rv|it|ra|ie)[\/: ]([\d.]+)/) || [0, "0"])[1], webkit: /webkit/.test(userAgent), opera: /opera/.test(userAgent), msie: /msie/.test(userAgent) && !/opera/.test(userAgent), mozilla: /mozilla/.test(userAgent) && !/(compatible|webkit)/.test(userAgent), tt: /tencenttraveler/.test(userAgent), chrome: /chrome/.test(userAgent), firefox: /firefox/.test(userAgent), safari: /safari/.test(userAgent), gecko: /gecko/.test(userAgent), ie6: this.msie && this.version.substr(0, 1) == "6" },
        g: function(obj) {
            return typeof obj == "object" ? obj : document.getElementById(obj)
        },
        ...
        setStyle: function(ele, styles) {
            for (var i in styles) { ele.style[i] = styles[i] }
        },
        ...
        setOpacity: function(ele, level) {
            if (this.browser.msie && (!document.documentMode || document.documentMode < 9)) { ele.style.filter = "Alpha(opacity=" + level + ")" } else { ele.style.opacity = level / 100 }
        },
        getX: function(e) {
            var t = e.offsetLeft;
...
(function() {
    ...
    extend(cookie, {
        set: function(sName, sValue, iExpireSec, sDomain, sPath, bSecure) {
            ...
            if (sDomain != undefined) { oCookieArray.push("domain=" + sDomain) }
            if (sPath != undefined) { oCookieArray.push("path=" + sPath) }
            if (bSecure) { oCookieArray.push("secure") }
...
(function() {
    ...
    extend(event, {
        ...
        ready: function(fn) {
            bindReadyEvent();
            if (milo.isReady) { fn.call() } else {
                if (isFunction(fn)) { milo.readyFn.push(fn) }
            }
        }
    });
        ...
        decode: function(newopt) {
            ...
            if (typeof newopt == "object") {
                ...
                if (newopt instanceof Array) {
                    for (var i = 0; i < newopt.length; i++) { newopt[i] = milo.decode(newopt[i]) }
                    return newopt
                } else if (newopt instanceof RegExp) {
                    return newopt
                } else {
                    for (var i in newopt) { newopt[i] = milo.decode(newopt[i]) }
                    return newopt
                }
            }
            return newopt
...
(function() {
    ...
    function init(obj) {
        getDesc(obj, function(obj, descData) {
            ...
            if (flow.functions[0].sExtModuleId == null) { need("ams.flowengine", function(FlowEngine) { FlowEngine.submit(window["amsCfg_" + obj.flowId]) }) } else {
                ...
                need("ams." + modName, function() {
                    ...
                    if (isFunction(obj.modSubmit)) { obj.modSubmit(window[mn + "_" + obj.flowId]) }
                })
            }
        })
    }
    ...
    function submit(obj) {
        obj.modSubmit = function(modObj) {
            if (isFunction(modObj.submit)) { modObj.submit(obj.flowId) }
        };
        init(obj)
    }
    extend(milo.ams, {
        amsInit: function(amsActivityId, flowId, callback) {
            ...
            getAmsFile(amsActivityId, flowId, function(ams_actdesc) {
                ...
                cfg.iFlowId = flowId;
                if (flow.functions[0].sExtModuleId == null) { need("ams.flowengine", function(FlowEngine) { FlowEngine.submit(window["amsCfg_" + flowId]) }) } else {
                    var modName = flow.functions[0].method;
                    if (amsActivityId > 6686 && amsActivityId != 6688 && amsActivityId != 6701 && amsActivityId != 6731 && (modName == "share.microblogFix" || modName == "share.microblogUser" || modName == "share.qqgameFeed" || modName == "share.qqSignButton" || modName == "share.qqSignQueryTime" || modName == "share.qqSignRadio" || modName == "share.qzoneFix" || modName == "share.qzoneUser" || modName == "share.shareQueryHistory")) { flow.functions[0].method = "share.commShare" }
                    if ((amsActivityId == 6370 || amsActivityId == 6241 || amsActivityId == 3733) && (modName == "share.microblogFix" || modName == "share.microblogUser" || modName == "share.qqgameFeed" || modName == "share.qqSignButton" || modName == "share.qqSignQueryTime" || modName == "share.qqSignRadio" || modName == "share.qzoneFix" || modName == "share.qzoneUser" || modName == "share.shareQueryHistory")) { flow.functions[0].method = "share.commShare" }
                    need("ams." + flow.functions[0].method, function() {
                        ...
                        if (isFunction(callback)) { callback(window[mn + "_" + flowId]) }
                    })
                }
            })
        },
        amsSubmit: function(amsActivityId, flowId) {
            ...
            if (window.event && window.event.srcElement && window.event.srcElement != document || caller && caller.arguments[0]) {
                ...
                if (target) {
                    ...
                    try { _amsCFG.triggerSourceData = eval("(" + data + ")") } catch (e) { _amsCFG.triggerSourceData = data }
                }
            }
            amsInit(amsActivityId, flowId, function(obj) {
                if (isFunction(obj.submit)) { obj.submit(flowId) }
            })
        }
    })
})();
...
namespace("milo.ui");
(function() { extend(milo.ui, { alert: function(msg) { alert(msg) } }) })(); /*  |xGv00|d68f5f5bfa2a9482e5bc75ddc9937341 */
